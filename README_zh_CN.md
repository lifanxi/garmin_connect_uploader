...
- 基于 PySide6 的桌面 GUI。
- 用于脚本、批量同步、格式转换、备份、清理和诊断的 CLI。
- 支持 Columbus CSV、GPX、FIT 和 NMEA RMC 输入格式。
...
支持将已有 FIT 活动文件（`.fit`、`.FIT`）作为输入。程序会解析 Record 中的时间戳、坐标、高度、速度和方向，再进入与文本格式相同的去重和上传流程。
...
CLI 清理以及对“识别为本工具上传”的活动进行编辑等破坏性或修改性操作，都会在修改远端活动之前检查这个签名。CLI 清理只会删除已签名活动。GUI 维护对话框中可以显式勾选包含非本工具上传活动，但必须先接受额外警告。
...
启动时，GUI 会检查当前工作目录下的 `.garth_session`。如果存在有效的 Garmin 会话，登录区域会保持锁定，文件和维护操作会启用。如果没有有效会话，文件和维护操作会保持禁用，直到登录成功。
...
使用维护区域的 **配置** 可以为 GUI 的检查/运行操作设置 home city 和活动名称模板。留空时保持默认行为。GUI 会把这些值保存到工作目录下的 `settings.yaml`。

使用 **清理已上传活动** 删除由本工具上传的远端活动。GUI 会先在确认对话框中预览匹配活动，删除前需要输入 `DELETE`。对话框中还有一个默认未勾选的选项，可以包含非本工具上传活动；启用该选项后，扫描前需要接受额外警告。
...
将 Garmin Connect 活动下载为原始轨迹文件。若 Garmin 返回的 ZIP 包中只有一个支持的文件，会按原格式保存该文件；
若 ZIP 包中包含多个文件，则直接保留原始 ZIP。支持的轨迹格式包括 `.tcx`、`.fit`、`.gpx`、`.xls`、`.xlsx`、`.csv` 和 `.xml`；多文件包会保存为 `.zip`。输出文件名使用 `YYYYMMDD_HHMMSS_activityId_活动名称.扩展名`，活动名称中不适合做文件名的字符和 GCU Token 会从备份文件名中移除。
