...
- 不在第一阶段实现所有轨迹格式，只设计可扩展边界并实现 Columbus CSV。
...
CLI 是主入口，支持同步、预览、转换、回填、清理和诊断：

```bash
gcu sync tracks/*.CSV
gcu sync tracks/*.CSV --dry-run
gcu sync tracks/*.CSV --domain garmin.cn
gcu sync tracks/*.CSV --timezone UTC
gcu sync tracks/*.CSV --display-timezone auto
gcu sync tracks/*.CSV --display-timezone Asia/Shanghai
gcu sync tracks/*.CSV --display-city auto
gcu sync tracks/*.CSV --display-city-min-population 300000
gcu sync tracks/*.CSV --format columbus-csv

gcu convert input.CSV --output output.fit
gcu inspect input.CSV
gcu backfill tracks/*.CSV
gcu purge --dry-run
gcu purge --yes
gcu auth login --domain garmin.cn
gcu auth status
```

`gcu purge` 会扫描 Garmin Connect 活动列表，仅删除签名同时满足
`manufacturer=HOLUX` 和 `deviceId=0x12345678` 的活动。默认全时间范围扫描；
可用 `--start-date YYYY-MM-DD` 和 `--end-date YYYY-MM-DD` 缩小范围。该命令是破坏性
操作，必须显式传入 `--yes`；预览使用 `--dry-run`。
...
GUI 是 CLI 的封装，不直接实现业务规则。

推荐两种实现路径：

1. 第一阶段：GUI 调用 `gcu` CLI 子进程，读取 JSON 事件流更新界面。
2. 第二阶段：GUI 直接调用 `gcu.app` 服务，但仍共享同一套命令模型和事件模型。

GUI 功能边界：

- 选择文件或文件夹。
- 设置 Garmin 域名、源时区、dry-run、是否 backfill。
...
```text
GPS Track - Nov 01 5h 35m [gcu:v1:9f2a6c4e81d3b7a0]
```
...
```text
GPS Track - Nov 01 5h 35m [gcu:v1:...]
```
...
```bash
gcu sync tracks/*.CSV --keep-fit --output-dir ./fit-out
gcu convert tracks/*.CSV --output-dir ./fit-out
```
...
default_format = auto
activity_name_template = GPS Track - {date} {duration}
